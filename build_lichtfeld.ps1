...
function Find-VSInstallPath {
    ...
    try {
        ...
        $VSVersion = & $VSWherePath -latest -products Microsoft.VisualStudio.Product.Community, Microsoft.VisualStudio.Product.Professional, Microsoft.VisualStudio.Product.Enterprise -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationVersion 2>$null
        ...
        if (-not $VSPath) {
            ...
            $VSVersion = & $VSWherePath -latest -products Microsoft.VisualStudio.Product.BuildTools -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationVersion 2>$null
        }

        # Return both path and major version
        $MajorVersion = 0
        if ($VSVersion) {
            $MajorVersion = [int]($VSVersion.Split('.')[0])
        }
        return @{ Path = $VSPath; MajorVersion = $MajorVersion }
    }
    catch {
        return $null
    }
...
function Launch-VSDevEnvironment {
    ...
    # Always pass -NoDevShellRelaunch to prevent infinite re-launch loops
    $ParamString += ' -NoDevShellRelaunch'

    # Create temp script to launch dev shell and run build
    $TempScript = Join-Path $env:TEMP "lichtfeld_build_$(Get-Random).ps1"

    # VS 2019 (major version 16) doesn't support -Arch/-HostArch parameters
    if ($VSMajorVersion -ge 17) {
        $DevShellArgs = "& '$VSDevShellScript' -Arch amd64 -HostArch amd64"
    }
    else {
        $DevShellArgs = "& '$VSDevShellScript'"
    }
...
if (-not (Test-VSDevEnvironment) -and -not $NoDevShellRelaunch) {
