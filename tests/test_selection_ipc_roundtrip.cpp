...
protected:
    ...
    auto invoke_and_process(F&& fn) {
        ...
        throw std::runtime_error("IPC command did not complete after command processing");
    }
...
TEST_F(SelectionIpcRoundTripTest, RectRoundTripUsesQueuedCommandProcessing) {
    ...
    auto result = invoke_and_process([this] {
        return client_->select_rect(0.0f, 0.0f, 30.0f, 30.0f, "replace", 0);
    });
...
TEST_F(SelectionIpcRoundTripTest, RingRoundTripUsesQueuedCommandProcessing) {
    ...
    auto result = invoke_and_process([this] {
        return client_->select_ring(50.0f, 50.0f, "replace", 0);
    });
    ASSERT_TRUE(result.has_value()) << result.error();
...
TEST_F(SelectionIpcRoundTripTest, BrushAndDeselectAllRoundTrip) {
    ...
    auto brush_result = invoke_and_process([this] {
        return client_->select_brush(10.0f, 10.0f, 8.0f, "replace", 0);
    });
    ASSERT_TRUE(brush_result.has_value()) << brush_result.error();
    EXPECT_EQ(selection_values(*scene_manager_), (std::vector<uint8_t>{1, 0}));

    auto clear_result = invoke_and_process([this] {
        return client_->deselect_all();
    });
